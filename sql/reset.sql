...
DROP FUNCTION IF EXISTS public.best_third_among_groups(text[], int[]) CASCADE;
DROP FUNCTION IF EXISTS public.best_third_among_groups(text[]) CASCADE;
DROP FUNCTION IF EXISTS public.r32_assigned_team_ids(int) CASCADE;
DROP FUNCTION IF EXISTS public.resolve_third_place_slot_team_id(text, int[]) CASCADE;
DROP FUNCTION IF EXISTS public.resolve_third_place_slots(boolean) CASCADE;
DROP FUNCTION IF EXISTS public.stage_predecessor(public.match_stage) CASCADE;
DROP FUNCTION IF EXISTS public.all_stage_matches_finished(public.match_stage) CASCADE;
DROP FUNCTION IF EXISTS public.all_stage_matches_have_teams(public.match_stage) CASCADE;
DROP FUNCTION IF EXISTS public.stage_ready_for_predictions(public.match_stage) CASCADE;
